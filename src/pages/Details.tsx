...
function Details() {
  ...
  const pricing = [
    { tier: 'Basic', price: '₹5000', features: ['2 Pages', 'SEO Optimised', '3 Revisions'] },
    { tier: 'Pro', price: '₹10000', features: ['5 Pages', 'SEO Optimised', 'Advanced Animations', '5 Revisions'] },
          ...
          <div className="brutal-card" style={{ background: 'var(--accent-blue)', color: 'white' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '2rem', alignItems: 'center' }}>
              ...
              <p style={{ fontSize: 'clamp(1rem, 3vw, 1.5rem)', fontWeight: 600 }}>
                I'm Ithish Jonnes—a designer who learned to code. I specialize in creating pixel-perfect layouts while solving complex backend problems, choosing the best tools for every project to ensure maximum performance. Whether you are a startup or an established brand, I build digital products that are as fast as they are beautiful. When I'm off the clock, I'm usually enjoying a perfect cup of coffee or exploring a new hiking trail.
              </p>
            </div>
          </div>
        </Reveal>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(clamp(250px, 30vw, 300px), 1fr))', gap: '2rem' }}>
          {projects.map((proj, idx) => (
            <Reveal key={idx} delay={idx * 0.12} direction="up">
              <div className="brutal-card" style={{ position: 'relative', height: '100%' }}>
                <div style={{ paddingBottom: '2rem' }}>
                  <h3 style={{ fontSize: 'clamp(1.25rem, 4vw, 1.75rem)', marginBottom: '0.5rem', wordWrap: 'break-word' }}>{proj.title}</h3>
          ...
          {pricing.map((tier, idx) => (
            <Reveal key={idx} delay={idx * 0.12} direction="up">
              <div className="brutal-card" style={{
                ...
                <h3 style={{ fontSize: 'clamp(1rem, 3vw, 1.25rem)', textTransform: 'uppercase', marginBottom: '1rem' }}>{tier.tier}</h3>
                <div style={{ fontSize: 'clamp(1.75rem, 5vw, 2.5rem)', fontWeight: 800, marginBottom: '2rem', fontFamily: 'Syne' }}>{tier.price}</div>
